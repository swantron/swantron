...
ZenpressPopup.prototype.insertImage = function(imageId,imageUrl,imageName) {
	...
	if (whatValue=="title") {
		...
		if (modRewrite) {
			path = webPath+"/"+albumUrl+"/image/thumb/"+imageUrl
		} else {
			path = webPath+"/setupzen/i.php?a="+albumUrl+"&amp;i="+imageUrl+"&amp;s=thumb";
		}
		imageHTML += '<img alt="'+imageName+'" title="'+imageName+'" src="'+path+'" />';
	}
	if (linkValue!="none") {
		imageHTML += '</a>';
	}
...
/**
 * Set up the status (expanded/collapsed) of a menu
 * @param obj	The toggle button for the menu
 */
ZenpressPopup.prototype.loadMenu = function(name) {
	//name = obj.id.substr(7);
	div = zenpressPopup.getEl('fields_'+name);
	hidden = zenpressPopup.getEl(obj.id+'_status');
	alert(hidden.value);
