...
if($mode == "act_OrderPages")
	$success = swiz_mypageorder_updateOrder();
...
<div id="page-order">
	...
	<h3><?php _e('Select Pages', 'mypageorder') ?></h3>
	    <p>Select the pages you want to be included in the navigation bar, click the order pages button below to save.</p>
	<ul id="order">
	<?php
	
	$inc=explode(',',get_option('swiz_page_inc'));
	$results = swiz_mypageorder_pageQuery();
...
// <![CDATA[
	...
	function orderPages() {
		...
		location.href = '<?php echo swiz_mypageorder_getTarget(); ?>?page=swiz-options&mode=act_OrderPages&parentID=<?php echo $parentID; ?>&idString='+idList+'&pageString='+pagesList+'#Header-Options';
	...
	function goEdit () {
		if(jQuery("#pages").val() != "")
			location.href="<?php echo swiz_mypageorder_getTarget(); ?>?page=swiz-options&mode=dsp_OrderPages&parentID="+jQuery("#pages").val();
	}
// ]]>
</script>
<?php
...
function swiz_mypageorder_getTarget() {
	return "admin.php";
}

function swiz_mypageorder_updateOrder()
{
	global $wpdb;	$pageList =  $_GET['pageString'];

	
	update_option( 'swiz_page_inc', $pageList );
	return '<div id="message" class="updated fade"><p>'. __('Pages Selected Successfully.', 'mypageorder').'</p></div>';
}


function swiz_mypageorder_pageQuery()
{
	global $wpdb;
...
function swiz_mypageorder_getParentLink($parentID)
{
	global $wpdb;
	...
	if($parentID != 0)
	{
		...
		return "<a href='". swiz_mypageorder_getTarget() . "?page=mypageorder&parentID=$parentsParent[0]'>" . __('Return to parent page', 'mypageorder') . "</a>";
	}
